...
### Q1 (basic)
Interview question: Explain portfolio weights, rebalancing, cumulative performance, and sql workflow in plain language for a trading or risk audience.

Model answer: A strong answer defines portfolio weights, rebalancing, cumulative performance, and sql workflow, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
Model answer: A strong answer includes Portfolio Return exactly, explains each symbol, and states one caveat: Using contemporaneous (leaky) weights.
...
Model answer: A strong answer uses one decision workflow such as: Portfolio backtest baseline.. Then it states one realistic failure mode: Using contemporaneous (leaky) weights., one detection check, and one mitigation action.
