...
### Q1 (basic)
Interview question: Explain mini project: compare three assets and explain diversification in plain language for a trading or risk audience.

Model answer: A strong answer defines mini project: compare three assets and explain diversification, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
### Q2 (intermediate)
Interview question: Write the Covariance formula/workflow from memory and define each symbol.

Model answer: A strong answer includes Covariance exactly, explains each symbol, and states one caveat: Miscaligned timestamps inflate noise.
...
Model answer: A strong answer uses one decision workflow such as: Diversification diagnostics.. Then it states one realistic failure mode: Miscaligned timestamps inflate noise., one detection check, and one mitigation action.
