...
### Q1 (basic)
Interview question: Explain review and policy checklist in plain language for a trading or risk audience.

Model answer: A strong answer defines review and policy checklist, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
Model answer: A strong answer uses one decision workflow such as: Quality control of extracted insights.. Then it states one realistic failure mode: High precision with very low coverage., and one detection check.
