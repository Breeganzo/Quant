...
### Q1 (basic)
Interview question: Explain decision trees and rule-based splits in plain language for a trading or risk audience.

Model answer: A strong answer defines decision trees and rule-based splits, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
### Q2 (intermediate)
Interview question: Write the Gini Impurity formula/workflow from memory and define each symbol.

Model answer: A strong answer includes Gini Impurity exactly, explains each symbol, and states one caveat: Overfitting deep trees without pruning.
How to answer in a live interview: State formula -> define symbols -> add caveat and context.
Why this matters: This checks mathematical fluency and operational reliability.

Python drill: Train a decision tree classifier on lagged ETF returns and print depth and accuracy.
...
Model answer: A strong answer uses one decision workflow such as: Split quality scoring.. Then it states one realistic failure mode: Overfitting deep trees without pruning., one detection check, and one mitigation action.
