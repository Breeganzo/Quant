...
### Q1 (basic)
Interview question: Explain turnover, holding period, and capacity intuition in plain language for a trading or risk audience.

Model answer: A strong answer defines turnover, holding period, and capacity intuition, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
Model answer: A strong answer uses one decision workflow such as: Execution quality benchmarking.. Then it states one realistic failure mode: Comparing fills to stale mid prices., one detection check, and one mitigation action.
