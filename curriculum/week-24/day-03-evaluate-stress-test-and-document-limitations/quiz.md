...
### Q1 (basic)
Interview question: Explain evaluate, stress test, and document limitations in plain language for a trading or risk audience.

Model answer: A strong answer defines evaluate, stress test, and document limitations, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
Model answer: A strong answer uses one decision workflow such as: Daily risk-limit reporting.. Then it states one realistic failure mode: Assuming normal tails in crisis regimes., one detection check, and one mitigation action.
