...
Model answer: A strong answer uses one decision workflow such as: Final capstone realism checks.. Then it states one realistic failure mode: Reporting only gross backtest outcomes., one detection check, and one mitigation action.
