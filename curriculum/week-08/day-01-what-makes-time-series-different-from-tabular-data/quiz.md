...
### Q1 (basic)
Interview question: Explain what makes time series different from tabular data? in plain language for a trading or risk audience.

Model answer: A strong answer defines what makes time series different from tabular data?, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
Model answer: A strong answer uses one decision workflow such as: Baseline dependence and mean-reversion diagnostics.. Then it states one realistic failure mode: Ignoring non-stationarity before fitting AR models., one detection check, and one mitigation action.
