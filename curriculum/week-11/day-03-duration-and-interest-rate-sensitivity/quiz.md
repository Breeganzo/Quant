...
Model answer: A strong answer uses one decision workflow such as: Interest-rate sensitivity baseline.. Then it states one realistic failure mode: Using duration for large non-linear shocks., one detection check, and one mitigation action.
