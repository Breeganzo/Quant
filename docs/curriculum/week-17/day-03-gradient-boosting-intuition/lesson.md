...
- Step 1 (Problem framing): Define the desk decision and why gradient boosting intuition is relevant.
...
- Step 3 (Method): Apply Additive Model or a directly related workflow on a reproducible sample.
- Step 4 (Result): Report one quantitative output and one practical interpretation for research, trading, or risk.
- Step 5 (Caveat): Document one failure mode: Using too high learning rate.
