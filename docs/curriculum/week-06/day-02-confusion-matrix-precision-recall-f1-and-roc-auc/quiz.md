...
### Q1 (basic)
Interview question: Explain confusion matrix, precision, recall, f1, and roc-auc in plain language for a trading or risk audience.

Model answer: A strong answer defines confusion matrix, precision, recall, f1, and roc-auc, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
Model answer: A strong answer includes Logistic Link exactly, explains each symbol, and states one caveat: Thresholding without business-cost calibration.
...
Model answer: A strong answer uses one decision workflow such as: Binary event prediction.. Then it states one realistic failure mode: Thresholding without business-cost calibration., one detection check, and one mitigation action.
