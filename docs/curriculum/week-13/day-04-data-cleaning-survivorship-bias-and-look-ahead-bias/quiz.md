...
### Q1 (basic)
Interview question: Explain data cleaning, survivorship bias, and look-ahead bias in plain language for a trading or risk audience.

Model answer: A strong answer defines data cleaning, survivorship bias, and look-ahead bias, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
### Q2 (intermediate)
Interview question: Write the Leakage Flag formula/workflow from memory and define each symbol.

Model answer: A strong answer includes Leakage Flag exactly, explains each symbol, and states one caveat: Using post-event data in training.
...
Model answer: A strong answer uses one decision workflow such as: Pipeline data audit.. Then it states one realistic failure mode: Using post-event data in training., one detection check, and one mitigation action.
