...
### Q1 (basic)
Interview question: Explain mini lab: compare momentum versus mean reversion rule in plain language for a trading or risk audience.

Model answer: A strong answer defines mini lab: compare momentum versus mean reversion rule, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
Model answer: A strong answer uses one decision workflow such as: Cross-sectional ranking inputs.. Then it states one realistic failure mode: Ignoring regime-dependent crash risk., one detection check, and one mitigation action.
