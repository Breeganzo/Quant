...
### Q1 (basic)
Interview question: Explain recommendation strategy and evidence package in plain language for a trading or risk audience.

Model answer: A strong answer defines recommendation strategy and evidence package, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
Model answer: A strong answer uses one decision workflow such as: Rank schools by evidence-based criteria.. Then it states one realistic failure mode: Using arbitrary weights without rationale., one detection check, and one mitigation action.
