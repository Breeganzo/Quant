...
### Q1 (basic)
Interview question: Explain ar, ma, and arima intuition in plain language for a trading or risk audience.

Model answer: A strong answer defines ar, ma, and arima intuition, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
### Q2 (intermediate)
Interview question: Write the ARIMA Model formula/workflow from memory and define each symbol.

Model answer: A strong answer includes ARIMA Model exactly, explains each symbol, and states one caveat: Choosing orders without diagnostics.
...
Model answer: A strong answer uses one decision workflow such as: Baseline univariate forecasting.. Then it states one realistic failure mode: Choosing orders without diagnostics., one detection check, and one mitigation action.
