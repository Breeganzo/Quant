...
### Q1 (basic)
Interview question: Explain functions, control flow, probability intuition, and expected value in plain language for a trading or risk audience.

Model answer: A strong answer defines functions, control flow, probability intuition, and expected value, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
Model answer: A strong answer includes Expected Value exactly, explains each symbol, and states one caveat: Ignoring payoff asymmetry and tails.
...
Model answer: A strong answer uses one decision workflow such as: Filter positive-edge trading setups.. Then it states one realistic failure mode: Ignoring payoff asymmetry and tails., one detection check, and one mitigation action.
