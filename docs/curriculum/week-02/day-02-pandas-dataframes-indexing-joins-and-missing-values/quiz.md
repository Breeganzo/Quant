...
### Q1 (basic)
Interview question: Explain pandas dataframes, indexing, joins, and missing values in plain language for a trading or risk audience.

Model answer: A strong answer defines pandas dataframes, indexing, joins, and missing values, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
### Q2 (intermediate)
Interview question: Write the Missingness Rate formula/workflow from memory and define each symbol.

Model answer: A strong answer includes Missingness Rate exactly, explains each symbol, and states one caveat: Forward-filling without documenting bias.
...
Model answer: A strong answer uses one decision workflow such as: Data quality monitoring before modeling.. Then it states one realistic failure mode: Forward-filling without documenting bias., one detection check, and one mitigation action.
