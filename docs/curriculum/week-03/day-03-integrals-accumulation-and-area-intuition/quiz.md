...
### Q1 (basic)
Interview question: Explain integrals, accumulation, and area intuition in plain language for a trading or risk audience.

Model answer: A strong answer defines integrals, accumulation, and area intuition, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
### Q2 (intermediate)
Interview question: Write the Definite Integral formula/workflow from memory and define each symbol.

Model answer: A strong answer includes Definite Integral exactly, explains each symbol, and states one caveat: Forgetting integration bounds context.
...
Model answer: A strong answer uses one decision workflow such as: Cumulative signal/flow interpretation.. Then it states one realistic failure mode: Forgetting integration bounds context., one detection check, and one mitigation action.
