...
### Q1 (basic)
Interview question: Explain mini lab: backtest a toy rule with costs in plain language for a trading or risk audience.

Model answer: A strong answer defines mini lab: backtest a toy rule with costs, gives one concrete workflow or market-facing decision example, and states one practical limitation that must be monitored.
...
### Q2 (intermediate)
Interview question: Write the Strategy Return with Costs formula/workflow from memory and define each symbol.

Model answer: A strong answer includes Strategy Return with Costs exactly, explains each symbol, and states one caveat: Ignoring costs in reported performance.
...
Model answer: A strong answer uses one decision workflow such as: Realistic backtest evaluation.. Then it states one realistic failure mode: Ignoring costs in reported performance., one detection check, and one mitigation action.
